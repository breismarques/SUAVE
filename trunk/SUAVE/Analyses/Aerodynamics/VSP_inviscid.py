...
# ----------------------------------------------------------------------
## @ingroup Analyses-Aerodynamics
class VSP_inviscid(Aerodynamics):
        ...
        if self.training_file is None:
            ...
            for i,_ in enumerate(AoA):
                for j,_ in enumerate(mach):
                    ...
                    CL[count],CD[count] = call_VSP(konditions, settings, geometry)
                    print CL[count]
                    print CD[count]
                    count += 1
...
def call_VSP(conditions,settings,geometry):
    ...
    # Run VSPAERO
    CL, CD = vspaero(tag + ".vsp3", conditions.aerodynamics.angle_of_attack, conditions.aerodynamics.mach, iters)
